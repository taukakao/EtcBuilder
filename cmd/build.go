...
func copyUserFile(relUserFile, oldSysDir, newSysDir, oldUserDir, newUserDir string, oldFileInfo os.FileInfo) error {
	...
	if slices.Contains(settings.SpecialFiles, relUserFile) {
		...
		return err
	} else if strings.HasPrefix(userFileType, "D") {
		fmt.Println("Keeping user device file", relUserFile)
		err := copyDeviceFile(absUserFileOld, absUserFileNew)
		return err
	} else {
...
func copyDeviceFile(fromPath, toPath string) error {
